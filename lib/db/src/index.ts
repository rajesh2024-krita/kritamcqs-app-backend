...
export async function connect(): Promise<{ databaseName: string; host: string }> {
  ...
  await ensureLearningAnalyticsIndexes();
...
async function ensureLearningAnalyticsIndexes() {
  const collection = mongoose.connection.db?.collection("chapterperformances");
  if (!collection) return;

  const indexes = await collection.indexes();
  const oldUnique = indexes.find((index) => index.name === "userId_1_chapterId_1" && Boolean(index.unique));
  if (oldUnique) {
    await collection.dropIndex(oldUnique.name);
  }
  await collection.createIndex(
    { userId: 1, chapterId: 1, topicId: 1 },
    { unique: true, name: "userId_1_chapterId_1_topicId_1" },
  );
}
...
export * from "./models/FreeQuestionConfig";
...
export * from "./models/SubscriptionPageTemplate";
...
export * from "./models/CmsMenuItem";
export * from "./models/WebsiteSettings";
export * from "./models/OfferTimerSettings";
export * from "./models/AppNotificationSettings";
export * from "./models/PushDeviceToken";
export * from "./models/NationalCompetition";
export * from "./models/NationalCompetitionRegistration";
export * from "./models/NationalCompetitionAttempt";
export * from "./models/NationalLeaderboardEntry";
