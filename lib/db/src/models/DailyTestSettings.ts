...
export interface IDailyTestSettings extends Document {
  ...
  allowBothExamsSameDay: boolean;
  subjectDistribution?: {
    NEET?: { Biology?: number; Chemistry?: number; Physics?: number };
    JEE?: { Mathematics?: number; Chemistry?: number; Physics?: number };
  };
...
DailyTestSettingsSchema.pre("validate", function validateDistribution(next) {
  ...
  const subjectDistribution = (this as any).subjectDistribution || {};
  const subjectGroups = [
    { label: "NEET", value: subjectDistribution.NEET },
    { label: "JEE", value: subjectDistribution.JEE },
  ];
  for (const group of subjectGroups) {
    const total = Object.values(group.value || {}).reduce((sum, value) => Number(sum) + Number(value || 0), 0);
    if (Number(total) > 0 && Number(total) !== Number(this.totalQuestions || 0)) {
      return next(new Error(`${group.label} subject-wise counts must equal total questions`));
    }
  }
