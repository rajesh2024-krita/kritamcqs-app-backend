...
export function getStrictQuestionExamModeValues(mode?: string) {
  const normalizedMode = normalizeMode(mode);
  if (normalizedMode === "JEE") return ["JEE", "JEE_MAIN", "JEE_ADVANCED"];
  if (normalizedMode === "NEET") return ["NEET", "NEET_UG"];
  return ["NEET", "JEE", "JEE_MAIN", "JEE_ADVANCED"];
}

export function buildStrictQuestionExamModeQuery(mode?: string) {
  const values = getStrictQuestionExamModeValues(mode);
  const baseModes = values.map((value) => (value.startsWith("JEE") ? "JEE" : value.startsWith("NEET") ? "NEET" : value));
  return {
    $or: [
      { examMode: { $in: [...new Set(baseModes)] } },
      { examType: { $in: [...new Set(baseModes)] } },
      { exam: { $in: values } },
    ],
  };
}
...
export async function getMixedSubjectIdsForExamMode(examMode?: string) {
  const normalizedMode = normalizeMode(examMode);
  const subjects = await Subject.find(buildManagedModeQuery(normalizedMode));
  return subjects
    .filter((subject) => isValidExamSubjectCombination(normalizedMode, subject.name))
    .map((subject) => String(subject._id));
}
