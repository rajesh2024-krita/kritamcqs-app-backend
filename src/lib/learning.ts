...
export function getLearningSourceInfo(session: {
  id?: string;
  origin?: string;
  title?: string;
  sourceSessionId?: string;
  ...
  const origin = String(session.origin || "practice_filter");
  const typeMap: Record<string, string> = {
    mock_test: "Mock Test",
    daily_set: "Daily Test",
    revision: "Revision Test",
    weak_area: "Practice Test",
    retest: "Practice Test",
    practice_filter: "Practice Test",
    smart_test: "Subject Test",
  };
  const sourceType = typeMap[origin] ?? "Practice Test";
  const snapshot = session.filterSnapshot ?? {};
  const rawName =
    session.title ||
    String(snapshot["sourceTestName"] || snapshot["mockTestTitle"] || snapshot["title"] || snapshot["dailyTestId"] || "").trim();
  const sourceName = rawName || sourceType;
  return {
    sourceType,
    sourceName,
    sourceLabel: sourceName.includes(sourceType) ? sourceName : `${sourceType} - ${sourceName}`,
