...
import { recordAffiliatePurchase, recordAffiliateSubscriptionAttempt } from "../services/affiliateService";
...
async function recordAffiliateAttemptForSubscription(req: AuthenticatedRequest, pendingSubscription: any, status: "PENDING" | "FAILED" | "CANCELLED", reason = "") {
  return recordAffiliateSubscriptionAttempt({
    userId: String(req.userId),
    subscriptionId: String(pendingSubscription?._id || ""),
    planId: String(pendingSubscription?.planId || ""),
    transactionId: String(pendingSubscription?.razorpayOrderId || pendingSubscription?._id || ""),
    platform: "ANDROID",
    paymentGateway: "razorpay",
    amount: Number(pendingSubscription?.amount || pendingSubscription?.finalAmount || 0),
    status,
    reason,
  }).catch((error) => req.log.warn({ err: error, subscriptionId: String(pendingSubscription?._id || "") }, "Affiliate subscription attempt recording failed"));
}
...
async function assertPurchaseProfileReady(userId: string) {
  const user = await User.findById(userId).select("mobile");
  const phone = String(user?.mobile || "").replace(/\D/g, "");
  if (!phone) {
    throw new Error("Please add your phone number before proceeding with the purchase.");
  }
}
...
function publicOrigin(req: AuthenticatedRequest) {
...
function publicUploadUrl(req: AuthenticatedRequest, value?: string) {
  const raw = String(value || "").trim();
  if (!raw) return "";
  if (/^https?:\/\//i.test(raw)) return raw;
  const normalizedPath = raw.startsWith("/") ? raw : `/${raw}`;
  return `${publicOrigin(req)}${normalizedPath}`;
}

async function readSavedInvoicePdf(pdfPath?: string) {
  const raw = String(pdfPath || "").trim();
  if (!raw || !raw.startsWith("/uploads/invoices/")) return null;
  try {
    return await fs.readFile(path.resolve(process.cwd(), raw.replace(/^\/+/, "")));
  } catch {
    return null;
  }
}

async function ensureInvoicePdf(invoice: any) {
  const existingPdf = await readSavedInvoicePdf(invoice.pdfPath);
  if (existingPdf) return existingPdf;
  return regenerateInvoicePdf(invoice, undefined, {}, { requireConnectedTemplate: true });
}

function mapInvoice(req: AuthenticatedRequest, invoice: any) {
  const raw = invoice.toJSON?.() || invoice;
  const pdfPath = raw.pdfPath || "";
  return {
    ...
    pdfPath,
    pdfUrl: publicUploadUrl(req, pdfPath),
...
router.get("/invoices", requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  res.json({ invoices: invoices.map((invoice) => mapInvoice(req, invoice)) });
});
...
router.get("/invoices/:id", requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  res.json(mapInvoice(req, invoice));
});
...
router.get("/invoices/:id/pdf", requireAuth, async (req: AuthenticatedRequest, res) => {
  ...
  const pdf = await ensureInvoicePdf(invoice);
  await invoice.save();

  if (String(req.query["link"] || "") === "1") {
    res.json({ invoice: mapInvoice(req, invoice), pdfUrl: publicUploadUrl(req, invoice.pdfPath) });
    return;
  }
...
router.post("/create-order", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    await assertPurchaseProfileReady(String(req.userId));
    ...
    await recordAffiliateAttemptForSubscription(req, pendingSubscription, "PENDING");
    ...
    const status = /coupon|plan|phone number/i.test(message) ? 400 : 500;
...
router.post("/verify-payment", requireAuth, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (!verifyRazorpaySignature(body.orderId, body.paymentId, body.signature, gateway.keySecret)) {
      ...
      await recordAffiliateAttemptForSubscription(req, pendingSubscription, "FAILED", "Razorpay payment signature verification failed");
    ...
    if (String(payment.order_id || "") !== String(body.orderId)) {
      ...
      await pendingSubscription.save();
      await trackFailedPaymentReminder(req, pendingSubscription, "payment_failed", plan.name);
      await recordAffiliateAttemptForSubscription(req, pendingSubscription, "FAILED", "Razorpay payment does not belong to this order");
    ...
    if (!Number.isFinite(paidAmount) || paidAmount <= 0) {
      ...
      await pendingSubscription.save();
      await trackFailedPaymentReminder(req, pendingSubscription, "payment_failed", plan.name);
      await recordAffiliateAttemptForSubscription(req, pendingSubscription, "FAILED", "Unable to verify Razorpay payment amount");
    ...
    if (paidAmount < expectedAmount) {
      ...
      await pendingSubscription.save();
      await trackFailedPaymentReminder(req, pendingSubscription, "payment_failed", plan.name);
      await recordAffiliateAttemptForSubscription(req, pendingSubscription, "FAILED", "Razorpay payment amount does not match this order");
    ...
    if (String(payment.status || "").toLowerCase() !== "captured") {
      ...
      await pendingSubscription.save();
      await trackFailedPaymentReminder(req, pendingSubscription, "payment_failed", plan.name);
      await recordAffiliateAttemptForSubscription(req, pendingSubscription, "FAILED", `Razorpay payment is not captured. Current status: ${payment.status || "unknown"}`);
