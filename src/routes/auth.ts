...
router.get("/settings", async (_req, res) => {
  ...
  const iosClientId = String(settings.googleIosClientId || process.env["GOOGLE_IOS_CLIENT_ID"] || "").trim();
  ...
  res.json({
    ...
    consent: {
      ...
      requiredForLogin: settings.consentRequiredForLogin !== false,
      requiredForSignup: settings.consentRequiredForSignup !== false,
      text: settings.consentText || "I agree to the Terms & Conditions and Privacy Policy.",
      buttonBehavior: settings.consentButtonBehavior || "disable_until_checked",
      policySlugs: Array.isArray(settings.consentPolicySlugs) && settings.consentPolicySlugs.length ? settings.consentPolicySlugs : ["terms", "privacy"],
      links: (Array.isArray(settings.consentPolicySlugs) && settings.consentPolicySlugs.length ? settings.consentPolicySlugs : ["terms", "privacy"]).map((slug: string) => ({
        slug,
        label: slug === "terms" ? "Terms & Conditions" : slug === "privacy" ? "Privacy Policy" : slug.replace(/-/g, " ").replace(/\b\w/g, (letter) => letter.toUpperCase()),
        href: `/policy/${slug}`,
      })),
    },
  });
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user.passwordHash || !verifyPassword(password, user.passwordHash)) {
      res.status(401).json({ error: "invalid_password", message: "Account found, but password is incorrect." });
      return;
    }
...
router.post("/apple", async (req, res) => {
  try {
    ...
    if (!checkRateLimit(`apple:${appleUserId}`, 12)) {
    ...
    if (firebaseUid) identityQueries.push({ firebaseUid });
    ...
    if (!user) {
      ...
      user.appleId = user.appleId || appleUserId;
