...
function questionExactMatch(value?: unknown) {
...
router.get("/", requireAuth, async (req, res) => {
  ...
  const exact = req.query["exact"] ?? (req.query["exactMode"] === "true" ? "true" : undefined);
  ...
  if (String(exact ?? "").trim().toLowerCase() !== "true") {
    yearDocs
      .filter((item: any) => matchesYearMode(item, mode))
      .forEach((item) => mergeYear(yearsByValue, item));
  }

  const questionFilters = [questionModeMatch(mode), questionExactMatch(exact)].filter((item) => Object.keys(item).length);
  const questionRows = await Question.find(questionFilters.length ? { $and: questionFilters } : {})
    .select("yearId year examYear previousYear examMode exam examType exact")
    .limit(5000)
    .lean();
