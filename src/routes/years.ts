...
function buildPaperModeFilter(mode: string) {
  ...
  if (mode === "JEE") {
    return {
      $or: [
        { examMode: "JEE" },
        { exam: { $in: ["JEE", "JEE_MAIN", "JEE_ADVANCED"] } },
      ],
    };
  }

  if (mode === "BOTH") {
    return { examMode: "BOTH" };
  }
...
router.get("/", requireAuth, async (req, res) => {
  try {
    ...
    // Get all available yearIds from questions
    const usedYearIds = await Question.distinct("yearId", {
      ...questionFilter,
      yearId: { $nin: [null, ""] },
    });

    // Compare Question.yearId with Year._id
