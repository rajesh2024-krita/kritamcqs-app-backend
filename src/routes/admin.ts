...
const WEBSITE_ASSET_ROOT = path.resolve(process.cwd(), "uploads");
...
const SAFE_UPLOAD_FOLDERS = new Set(["app-assets", "website-content"]);
const QUESTION_COLUMNS = {
  ...
  exact: ["exact", "is_exact", "isExact", "pyq_exact", "pyqExact"],
} as const;
...
function normalizeOverviewExamMode(value: unknown): "NEET" | "JEE" | "BOTH" {
  const normalized = String(value ?? "").trim().toUpperCase();
...
function overviewModeAllows(userMode: unknown, itemMode: unknown) {
  const normalizedUserMode = normalizeOverviewExamMode(userMode);
  const normalizedItemMode = normalizeOverviewExamMode(itemMode);
  return normalizedUserMode === "BOTH" ? normalizedItemMode === "NEET" || normalizedItemMode === "JEE" : normalizedItemMode === normalizedUserMode;
}
...
function normalizeUserPayload(body: Record<string, unknown>) {
  ...
  return {
    mobile: String(body.mobile ?? "").trim() || undefined,
...
async function normalizeQuestionPayload(body: Record<string, unknown>) {
  ...
  return {
    ...
    exact: body.exact === undefined ? undefined : parseBoolean(body.exact),
  };
}
...
async function getUserOverview(userId: string) {
  ...
  const [
    ...
    ChapterPerformance.find({ userId: id }).sort({ accuracy: 1, updatedAt: -1 }).limit(100),
    Subject.find(),
    Chapter.find(),
    Topic.find(),
  ...
  const filteredWeakAreas = weakAreas
    .map((area) => {
      ...
      const examType = normalizeOverviewExamMode(subject?.examType ?? subject?.examMode);
      return { area, subject, examType };
    })
    .filter((item) => overviewModeAllows(user.examMode, item.examType))
    .slice(0, 20);
  ...
  return {
    ...
    mistakes: mistakes.map((mistake) => {
      ...
      const topic = topicMap.get(String(question?.topicId ?? mistake.topicId));
    ...
    weakAreas: filteredWeakAreas.map(({ area, subject, examType }) => {
      const chapter = chapterMap.get(String(area.chapterId));
      const topic = topicMap.get(String((area as any).topicId));
      return {
...
function normalizeUploadFolder(value: unknown) {
  const folder = String(value || "app-assets").trim().replace(/\\/g, "/");
  return SAFE_UPLOAD_FOLDERS.has(folder) ? folder : "app-assets";
}

async function saveUploadedImage(file: Express.Multer.File, folderValue: unknown) {
  ...
  const folder = normalizeUploadFolder(folderValue);
  const uploadDir = path.join(WEBSITE_ASSET_ROOT, folder);
  await fs.mkdir(uploadDir, { recursive: true });
  const extension = inferImageExtension(file.originalname || "", file.mimetype || "");
  const baseName = path.basename(file.originalname || "image", path.extname(file.originalname || "")).replace(/[^a-zA-Z0-9-_]/g, "-").replace(/-+/g, "-").slice(0, 64) || "image";
  const filename = `${baseName}-${Date.now()}-${Math.random().toString(16).slice(2)}${extension}`;
...
async function resolveBulkRow(
  ...
  const payload = await normalizeQuestionPayload({
    ...
    exact: getCell(row, QUESTION_COLUMNS.exact),
  });
...
router.get("/free-question-configs", wrap(async (_req, res) => {
  const configs = await FreeQuestionConfig.find({}).sort({ createdAt: -1 });
  const subjectIds = [...new Set(configs.map((item) => String(item.subjectId)).filter(Boolean))];
  const subjects = subjectIds.length ? await Subject.find({ _id: { $in: subjectIds } }) : [];
  const subjectMap = new Map(subjects.map((item) => [String(item._id), item]));
  sendSuccess(res, configs.map((item) => ({
    ...(typeof item.toJSON === "function" ? item.toJSON() : item),
    subjectName: subjectMap.get(String(item.subjectId))?.name ?? "-",
  })));
}));

router.post("/free-question-configs", wrap(async (req, res) => {
  const subjectId = String(req.body?.subjectId ?? "").trim();
  if (!subjectId) throw Object.assign(new Error("Subject is required"), { statusCode: 400 });
  const selectionMode = String(req.body?.selectionMode ?? "automatic") === "manual" ? "manual" : "automatic";
  const payload = {
    subjectId,
    selectionMode,
  ...
  sendSuccess(res, item, { status: 201, message: "Free question configuration saved" });
}));

router.delete("/free-question-configs/:id", wrap(async (req, res) => {
  assertObjectId(String(req.params.id), "free question config");
  await FreeQuestionConfig.findByIdAndDelete(req.params.id);
  sendSuccess(res, null, { message: "Free question configuration deleted" });
}));
...
router.put("/users/:id", wrap(async (req, res) => {
  ...
  if (!item) throw Object.assign(new Error("User not found"), { statusCode: 404 });
  const payload = normalizeUserPayload(req.body ?? {});
  if (req.body?.mobile !== undefined) item.mobile = payload.mobile as any;
...
router.get("/website-content/landing", wrap(async (_req, res) => {
  const item = await WebsiteContent.findOneAndUpdate(
    { key: "landing" },
    { $setOnInsert: { key: "landing", content: {}, status: "draft" } },
...
router.put("/website-content/landing", wrap(async (req, res) => {
  const body = req.body ?? {};
  ...
  sendSuccess(res, {
    ...
    publishedAt: item.publishedAt,
    updatedAt: item.updatedAt,
  }, { message: status === "published" ? "Website content published" : "Website content saved" });
}));

router.post("/uploads/app-image", upload.single("image"), wrap(async (req, res) => {
  const url = await saveUploadedImage(req.file as Express.Multer.File, req.body?.folder);
  sendSuccess(res, { url, path: url }, { status: 201, message: "Image uploaded" });
}));

router.post("/uploads/app-images", upload.array("images", 20), wrap(async (req, res) => {
  const files = Array.isArray(req.files) ? req.files as Express.Multer.File[] : [];
  if (!files.length) throw Object.assign(new Error("At least one image file is required"), { statusCode: 400 });
  const images = [];
  for (const file of files) {
    const url = await saveUploadedImage(file, req.body?.folder);
    images.push({ url, path: url });
  }
  sendSuccess(res, { images }, { status: 201, message: "Images uploaded" });
}));

router.post("/app-settings/logo", upload.single("logo"), wrap(async (req, res) => {
  ...
  const extension = inferImageExtension(req.file.originalname || "", req.file.mimetype || "");
  const fileName = `app-logo-${Date.now()}${extension}`;
